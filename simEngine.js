...
// ================================
// Removes the incorrect flag reset from Company.step() to fix the state
// management conflict that was causing the game loop to halt.
// -------------------------------------------------------------------
...
class Stage {
  ...
  advance (dt, rng, company, prod) {
    ...
    if (this.elapsed >= this.duration_days) {
      ...
      console.log(`[${company.name}] ${prod} – ${this.name} ${this.succeeded ? 'succeeded' : 'failed'}`);
    }
  }
}
class Product {
...
/*──────────────────── Company ─────────────────────*/
class Company {
  constructor (cfg, gameStartYear = 1990, ipoDate = new Date(gameStartYear, 0, 1)) {
    this.id = cfg.id; this.name = cfg.static.name; this.sector = cfg.static.sector;
    this.ipoDate = ipoDate; // Store ipoDate on the Company instance
    ...
    this.startYear = gameStartYear; this.currentYearRevenue = 0; this.currentYearProfit = 0; this.lastYearEnd = 0; this.financialHistory = [];
    ...
    // Pre-simulate if IPO date is before game start
    let currentSimDate = new Date(ipoDate);
    const dt = 14; // Use the same dt as the main simulation for pre-simulation

    while (currentSimDate.getFullYear() < gameStartYear) {
        this.step(dt, currentSimDate);
        currentSimDate.setDate(currentSimDate.getDate() + dt);
    }
  }

  step (dt, gameDate) {
    // THIS LINE WAS REMOVED: `this.hasPipelineUpdate = false;`
    // The simulation engine's job is only to set the flag to TRUE when an update
    // happens. The UI (the "consumer") is responsible for setting it back to false.
    // Leaving this line in created a race condition that could halt the game loop.
    ...
    for (const p of this.products) { const revBefore = p.realised(); p.advance(dt, Math.random, this); if (p.realised() > revBefore) successThisTick = true; }
...
class Simulation {
    constructor(cfg, startYear = 1990, dt = 14) {
        ...
        this.companyConfigs = cfg.map(c => ({ ...c, isLive: false, ipoDate: null })); // Store raw configs with state
        this.companies = []; // This will hold active, public companies
    ...
    tick(gameDate) {
        ...
        this.companyConfigs.forEach(config => {
            if (config.isLive) return; // Skip if already public
            ...
            if (!config.ipoDate) {
                if (config.static.ipo_instantly) {
                    ...
                    const ipoYear = Math.floor(between(from, to + 1)); // Random year in [from, to]
                    const ipoDayOfYear = Math.floor(between(1, 366)); // Random day of the year
                    
                    // Create the date object
                    const date = new Date(ipoYear, 0, 1); // Start with Jan 1st of the IPO year
                    date.setDate(ipoDayOfYear); // Set the random day
                    config.ipoDate = date;
                }
            }

            if (gameDate >= config.ipoDate) {
                const newCompany = new Company(config, this.startYear, config.ipoDate);
                this.companies.push(newCompany);
                config.isLive = true; // Mark as live so it's not processed again
                console.log(`[IPO] ${newCompany.name} went public on ${gameDate.toISOString().split('T')[0]}`);
            }
        });

        // Filter out the live configs to improve performance on subsequent ticks
        // This is an optimization to avoid iterating over companies that are already public
        this.companyConfigs = this.companyConfigs.filter(c => !c.isLive);

        // Step the active (public) companies
